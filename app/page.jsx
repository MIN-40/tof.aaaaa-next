...
export default function Home() {
  ...
  const edgeScrollRef = useRef({ key: '', time: 0, count: 0, lastCountedAt: 0 });
    ...
    const onWheel = (event) => {
      ...
      if (currentSectionRef.current === 1) {
        ...
        if (canScrollInside(event.target, direction)) {
          edgeScrollRef.current = { key: '', time: 0, count: 0, lastCountedAt: 0 };
          return;
        }
        if (scrollable) {
          ...
          const isSameEdge = previous.key === edgeKey && now - previous.time < 2400;
          const isSeparateGesture = !isSameEdge || now - previous.lastCountedAt > 420;
          const nextCount = isSameEdge ? previous.count + (isSeparateGesture ? 1 : 0) : 1;
          const canLeaveProductPage = nextCount >= 4;
          edgeScrollRef.current = { key: edgeKey, time: now, count: nextCount, lastCountedAt: isSeparateGesture ? now : previous.lastCountedAt };
